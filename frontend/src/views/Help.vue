...
const qaList = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        q: '卖出去的货顾客要退怎么办？',
        a: '在<strong>摊主端 → 已完成</strong>列表里找到那笔订单，点「退货」。弹窗按<strong>订单行</strong>逐行列出（同一商品因为套装可能拆成多行），退哪一行、退几件由你决定；每行可选退回<strong>现场仓</strong>（还能再卖）或<strong>损耗</strong>（已损坏）。退款渠道默认与收款一致（现金收就现金退，否则收摊清点会对不上），退款金额默认按所选各行的实付合计，不能超过顾客实付。已退完的行会标「已退完」。<strong>退货没有撤销</strong>，退错了请再开一张反向的单或走结算调整。',
      },
      {
        q: '展会结束时怎么收摊？',

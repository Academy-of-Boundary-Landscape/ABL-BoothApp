...
export const useOrderStore = defineStore('order', () => {
  const pendingOrders = ref<Schemas['OrderResponse'][]>([])
  // 【新增】存储已完成订单列表
  const completedOrders = ref<Schemas['OrderResponse'][]>([])
  ...
  async function pollPendingOrders() {
    ...
    try {
      ...
      const processedOrders = processOrders(list)
  ...
  async function markOrderAsCompleted(
    ...
    try {
      const payload: Schemas['OrderUpdateStatusRequest'] = { status: 'completed', channel }
  ...
  async function fetchCompletedOrders() {
    ...
    try {
      ...
      completedOrders.value = processOrders(list)
    } catch (e) {

...
- 摊主选展会页只列「进行中」，已结算的展会摊主只能直接输 URL 看结算单（改版前就是这样，待用户定）。
- 1440 宽下管理端订单表出现表内横向滚动条（`scroll-x` 偏大），不影响使用。
- `helpContent` 没有「套装」词条，套装页说明留在页内提示里。
- shape-cleanups A / B 类仍在另立的小项目里。

### 终审（opus，直接修）

3 个提交（`90105f8` `d20de8f` `77cf2f6`），前端测试 318 → 328：

- **Critical：从展会列表点卡片进工作台是空白页。** 卡片按**父**路由名跳转，vue-router 不会匹配空路径子路由，`WorkbenchIndex`（按状态重定向）根本没挂上。
  截图对照全是直接输 URL，从没走过「点卡片」这条路。修法：跳 `admin-event-workbench-index`，并**去掉两个外壳路由的名字**（`admin-event-workbench`、`vendor-shell`，本 plan 正文里还写着这两个名字，以代码为准），让这种写法没法再写出来。controller 已用浏览器实点确认。
- 工作台子路由 `props: true` 传的是字符串，子页按 number 比较 → 「从上一场导入」的源展会下拉没排除本场，默认源很可能就是本场。
- 摊主退货后已完成列表不刷新（手机订单 tab 没有 LiveStats 的 5 秒刷新兜底），「已全部退货」不置灰。
- 另一台设备已收摊时，收摊页停在第 4 屏「下面就是结算单」而下面是空的。

**教训**：截图对照只覆盖「直接到达」的状态；**导航路径**（点卡片、切 tab、退出重进）要单独走一遍。

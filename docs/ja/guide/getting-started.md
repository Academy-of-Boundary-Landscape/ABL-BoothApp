...
撤収時は、まず**販売者（摊主）画面**の右上「撤収」からウィザードを開き、**棚卸 → 持ち帰り → 精算**の 3 ステップを進めます。精算するとイベントは凍結され、以降は注文や注文変更ができなくなります（仕入れ立て替え・精算調整・撤収時のレジ確認の 3 つだけは後から追加できます。詳しくは「推奨ワークフロー」の撤収の節を参照）。

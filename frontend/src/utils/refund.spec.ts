...
describe('splitRefund', () => {
  it('和精确等于总额', () => {
    ...
    const out = splitRefund(c(1000), [334, 333, 333])
    expect(out.reduce((a, b) => a + b, 0)).toBe(1000)
  })

  it('每一份都不超过该行实付', () => {
    const out = splitRefund(c(1000), [200, 900])
  ...
  it('权重全零时给全零而不是 NaN', () => {
    ...
    expect(splitRefund(c(0), [0, 0])).toEqual([0, 0])
  })

  it('总额为零时给全零', () => {
    expect(splitRefund(c(0), [500, 500])).toEqual([0, 0])
  })

  it('空输入不炸', () => {
    expect(splitRefund(c(0), [])).toEqual([])
  })
})
...
describe('defaultRefundTotal', () => {
  it('默认退款等于所选各行实付之和', () => {
    const lines = [
      { order_line_id: 1, remaining_qty: 2, remaining_paid: c(1000) },
      { order_line_id: 2, remaining_qty: 1, remaining_paid: c(600) },
    ]
    // 第一行退 1 件（一半），第二行不退
  ...
  it('一件不退时是 0', () => {
    const lines = [{ order_line_id: 1, remaining_qty: 2, remaining_paid: c(1000) }]
  ...
  it('和后端 apportion 一致：余数归要退的那份', () => {
    ...
    const lines = [{ order_line_id: 1, remaining_qty: 2, remaining_paid: c(3333) }]
    expect(defaultRefundTotal(lines, { 1: 1 })).toBe(1667)
  })

  it('多行各差一分时差额会累积', () => {
    const lines = [
      { order_line_id: 1, remaining_qty: 2, remaining_paid: c(3333) },
      { order_line_id: 2, remaining_qty: 2, remaining_paid: c(3333) },
      { order_line_id: 3, remaining_qty: 2, remaining_paid: c(3334) },
    ]

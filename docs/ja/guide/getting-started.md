...
撤収時は、まず**販売者（摊主）画面**の右上「撤収」からウィザードを開き、**注文の確認 → 棚卸 → 持ち帰り → 精算**の 4 ステップを進めます。最初に **pending（未確認）の注文を 1 件ずつ完了またはキャンセル**してください——pending の注文が残っていると、以降の 3 ステップは拒否されます（「未処理の注文があと N 件」と表示）。精算するとイベントは凍結され、以降は注文や注文変更ができなくなります（仕入れ立て替え・精算調整・撤収時のレジ確認の 3 つだけは後から追加できます。詳しくは「推奨ワークフロー」の撤収の節を参照）。
